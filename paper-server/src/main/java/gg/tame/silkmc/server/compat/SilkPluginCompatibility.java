...
public final class SilkPluginCompatibility {
    ...
    public static void inspectAndEnforce(final Path source, final io.papermc.paper.plugin.configuration.PluginMeta meta) {
        SilkPluginCompatibilityManager.inspectAndEnforce(source, meta);
    }
    ...
    public static boolean shouldWarnOnLoad() {
        return policy().warnOnLoad;
    }

    public static boolean markWarned(final String pluginName) {
        return WARNED_PLUGINS.add(normalizePluginName(pluginName));
    }

    public static @org.jetbrains.annotations.Nullable OverrideEntry overrideFor(final String pluginName) {
        return policy().overrides.get(normalizePluginName(pluginName));
    }
    ...
    private static void enforce(final String pluginName, final boolean supported) {
        ...
        if (policy.warnOnLoad && WARNED_PLUGINS.add(normalizePluginName(pluginName))) {
    ...
    private static Policy loadPolicy() {
        ...
        return new Policy(
            ...
            loadOverrides(overridesConfig)
        );
    }

    private static Map<String, OverrideEntry> loadOverrides(final YamlConfiguration config) {
        final ConfigurationSection section = config.getConfigurationSection("overrides");
        if (section == null) {
            return Map.of();
        }

        final Map<String, OverrideEntry> overrides = new LinkedHashMap<>();
        for (final String pluginName : section.getKeys(false)) {
            final String basePath = "overrides." + pluginName;
            overrides.put(
                normalizePluginName(pluginName),
                new OverrideEntry(
                    pluginName,
                    ClassificationOverride.from(config.getString(basePath + ".classification", "AUTO")),
                    config.getBoolean(basePath + ".force-load", false),
                    Objects.toString(config.getString(basePath + ".reason", ""), "")
                )
            );
        }
    ...
    private static String normalizePluginName(final String pluginName) {
        return pluginName.trim().toLowerCase(Locale.ROOT);
    }

    private static void saveConfig(final YamlConfiguration config, final File target, final String description) {
        try {
            config.save(target);
        } catch (final IOException ex) {
            LOGGER.log(Level.WARNING, "Failed to write " + description + " to " + target.getAbsolutePath(), ex);
        }
    }
    ...
    private record Policy(
        ...
        Map<String, OverrideEntry> overrides
    ) {
    }

    public record OverrideEntry(String pluginName, ClassificationOverride classificationOverride, boolean forceLoad, String reason) {
    }

    public enum ClassificationOverride {
        AUTO,
        SAFE,
        COMPATIBLE,
        UNKNOWN,
        ...
        private static ClassificationOverride from(final String value) {
            try {
                return ClassificationOverride.valueOf(value.trim().toUpperCase(Locale.ROOT));
            } catch (final IllegalArgumentException ex) {
                return AUTO;
            }
        }

        public SilkPluginCompatibilityManager.Classification toClassification() {
            return switch (this) {
                case SAFE -> SilkPluginCompatibilityManager.Classification.SAFE;
                case COMPATIBLE -> SilkPluginCompatibilityManager.Classification.COMPATIBLE;
                case UNKNOWN -> SilkPluginCompatibilityManager.Classification.UNKNOWN;
                case UNSAFE -> SilkPluginCompatibilityManager.Classification.UNSAFE;
                case AUTO -> throw new IllegalStateException("AUTO is not an effective classification");
            };
        }
    }
